-- G2 payroll analytics: attribute each payroll fact to the department effective on competence day 1.
-- Assignment validity is half-open [effective_from, effective_to). Missing assignments fail closed:
-- the fact is not materialized and the operational drift check reports the data-quality gap.

create or replace view public.vw_analytics_folha_pagamento as
with payroll_facts as (
    select
        fp.*,
...
base as (
    ...
    from payroll_facts fp
    join public.funcionarios f on f.id = fp.funcionario_id
    join public.funcionario_lotacoes_departamento ld
      on ld.funcionario_id = fp.funcionario_id
     and ld.effective_from <= fp.competencia
     and coalesce(ld.effective_to, 'infinity'::date) > fp.competencia
    join public.departamentos d on d.id = ld.departamento_id
...
select
    ...
    case
        ...
        when cargo ilike '%repórter%' or cargo ilike '%reporter%' or departamento ilike '%comunica%' or departamento ilike '%jornalis%' then 'MEDIA'
...
grant select on public.vw_analytics_folha_pagamento to ${OPERATIONAL_RUNTIME_ROLE};
