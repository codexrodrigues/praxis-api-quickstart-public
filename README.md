...
- Praxis Config Starter (biblioteca)
  - Hospeda `/api/praxis/config/**`, `/api/praxis/runtime/context`, `/api/praxis/runtime/tenants`, `/api/praxis/runtime/navigation`, config-store transacional, AI registry, domain catalog, Project Knowledge, Domain Knowledge change sets e domain rules.
...
- Seguranca: `src/main/java/com/example/praxis/apiquickstart/config/SecurityConfig.java` - Swagger, Home e Health publicos; `/api/praxis/config/**` publico para config-store/IA; `/api/praxis/runtime/context`, `/api/praxis/runtime/tenants` e `/api/praxis/runtime/navigation` seguem a politica de leitura/autenticacao do host; demais rotas controladas por sessao JWT + flags `read-open`/whitelist.
...
- Config: `io.github.codexrodrigues:praxis-config-starter:0.1.0-rc.69`
...
- `io.github.codexrodrigues:praxis-config-starter` esta alinhado ao corte publicado `0.1.0-rc.69`; este quickstart acompanha o release candidate usado para validar os contratos atuais no host operacional de referencia e no rollout Render.
...
- Com `praxis-config-starter:0.1.0-rc.69`, o quickstart tambem prova `GET /api/praxis/runtime/context`, `GET /api/praxis/runtime/tenants` e `GET /api/praxis/runtime/navigation` com um provider demonstrativo nao-Ergon (`QuickstartEnterpriseRuntimeContextProvider`). Esse provider apenas projeta contexto publico seguro, uma lista de tenants demonstrativa e uma arvore de navegacao com refs canonicas Praxis para shell/AI grounding; autenticacao, autorizacao privada, roles reais, tenant entitlement e menus corporativos continuam sendo responsabilidade do host corporativo.
