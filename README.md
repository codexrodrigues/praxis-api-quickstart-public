...
- `io.github.codexrodrigues:praxis-config-starter` esta alinhado ao corte publicado `0.1.0-rc.112`. Alem da resolucao exata de referencias de templates governados, do lifecycle de snapshots e da evidencia operacional V57, esse corte oferece a primeira explicacao consultiva de uma decisao governada. O Policy Studio envia somente `selectedDomainDecisionRef`; o Config rele a versao exata, aplica a politica `governance.aiUsage`, projeta evidencia sanitizada e sempre encerra com `canApply=false`. O roteamento permanece semantico e authorado pela LLM: a referencia selecionada funciona como contexto governado, enquanto a tool canônica `inspectDomainDecision` produz a evidencia exata e evita que a explicacao seja contaminada por busca generica de recursos. O Quickstart continua sendo apenas o host de referencia e nao redefine essa semantica.
...
- `PraxisCockpitStarterConsumptionIntegrationTest`: valida que o host serve `/praxis/cockpit` e `/praxis/cockpit/index.html` a partir dos assets empacotados pelo `praxis-metadata-starter`, sem copiar cockpit para o quickstart. Tambem confirma os modos progressivos de visao geral, exploracao do dominio e detalhe do recurso. Esse teste e o gate downstream minimo para provar que hosts protegidos conseguem consumir a superficie publica do cockpit publicada pelo starter.
