...
- O alvo coordenado deste corte é `io.github.codexrodrigues:praxis-config-starter:0.1.0-rc.113`. Além da resolução exata de referências de templates governados, do lifecycle de snapshots e da explicação consultiva introduzida no rc.112, o rc.113 adiciona a evidência operacional V58: transporte leve, baseline independente por cenário, retry idempotente, vínculo do Test Run submetido e gates opt-in de `SUBMIT`/`PROMOTE`. O Quickstart V59/V60 publica a action host-owned que executa quatro cenários descartáveis `CREATE`/`UPDATE` × `ALLOW`/`DENY`, exige authority dedicada, ETag forte e idempotência, e persiste somente a evidência sanitizada no Config. A mutação de prova roda numa transação operacional rollback-only: o host captura before/after dentro dela e verifica o estado limpo depois do rollback, sem apagar ou contornar auditoria append-only. O Policy Studio envia somente `selectedDomainDecisionRef` ao assistente; o Config relê a versão exata, aplica `governance.aiUsage`, projeta evidência sanitizada e encerra a explicação com `canApply=false`. O roteamento permanece semântico e authorado pela LLM. O Quickstart continua sendo apenas o host de referência e não redefine essa semântica.
