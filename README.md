...
- `io.github.codexrodrigues:praxis-config-starter` esta alinhado ao corte publicado `0.1.0-rc.111`. Alem da resolucao exata de referencias de templates governados, do lifecycle de snapshots e da evidencia operacional V57, esse corte oferece a primeira explicacao consultiva de uma decisao governada. O Policy Studio envia somente `selectedDomainDecisionRef`; o Config rele a versao exata, aplica a politica `governance.aiUsage`, projeta evidencia sanitizada e sempre encerra com `canApply=false`. O Quickstart continua sendo apenas o host de referencia e nao redefine essa semantica.
