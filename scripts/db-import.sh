...
# Ou coloque o arquivo em ./db/init e suba o servico pela primeira vez (init automatico)
...
if [[ ${1:-} == "" ]]; then
  ...
  echo "Ex.: bash scripts/db-import.sh db/dump/public-demo-seed.sql" >&2
  exit 1
fi

DUMP_PATH=$1
if [[ ! -f "$DUMP_PATH" ]]; then
  echo "Arquivo nao encontrado: $DUMP_PATH" >&2
  exit 1
fi

if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
  echo "Container ${CONTAINER_NAME} nao esta em execucao. Suba com: docker compose up -d db" >&2
  exit 1
fi
