...
$workDirectory = Join-Path $projectRoot 'target\rule-audit-role-separation-proof'
$classpathFile = Join-Path $workDirectory 'test-classpath.txt'
$evidenceFile = Join-Path $workDirectory 'role-separation-evidence.json'
...
$directBranchHost = $BranchHost -replace '-pooler(?=\.)', ''
$jdbcUrl = "jdbc:postgresql://$directBranchHost$($connectionUri.AbsolutePath)?sslmode=require"
...
try {
    ...
    try {
        ...
        $env:ADR12_ROLE_JDBC_URL = $jdbcUrl
        $env:ADR12_ROLE_DATABASE_USER = $databaseUser
        $env:ADR12_ROLE_DATABASE_PASSWORD = $databasePassword
        $env:ADR12_ROLE_PROJECT_ROOT = $projectRoot
        $env:ADR12_ROLE_EVIDENCE_FILE = $evidenceFile
        $env:ADR12_ROLE_EPHEMERAL_BRANCH = 'true'

        & $java -cp $classpath com.example.praxis.apiquickstart.rulelab.RuleAuditRoleSeparationProof
        if ($LASTEXITCODE -ne 0) { throw 'ADR-12 role separation proof failed.' }
        if (-not (Test-Path -LiteralPath $evidenceFile)) {
            throw 'ADR-12 role separation proof produced no evidence file.'
        }
        Write-Host "ADR12_ROLE_SEPARATION_PROOF_PASS evidence=$evidenceFile"
    } finally {
        Pop-Location
    }
} finally {
    Remove-Item Env:ADR12_ROLE_JDBC_URL,Env:ADR12_ROLE_DATABASE_USER,Env:ADR12_ROLE_DATABASE_PASSWORD,`
        Env:ADR12_ROLE_PROJECT_ROOT,Env:ADR12_ROLE_EVIDENCE_FILE,Env:ADR12_ROLE_EPHEMERAL_BRANCH `
