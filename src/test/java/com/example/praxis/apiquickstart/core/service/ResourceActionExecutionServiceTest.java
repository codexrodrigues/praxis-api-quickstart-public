...
class ResourceActionExecutionServiceTest {
    @Test void replaysWinnerAfterUniqueKeyRace() {
        ...
        when(repository.findByResourceKeyAndResourceIdAndActionIdAndActorSubjectAndIdempotencyKey(
                "r", "__collection__", "a", "operator", "key")).thenReturn(Optional.of(winner));
    ...
    @Test void rejectsSameKeyWithDifferentCommand() {
        ...
        org.springframework.security.core.context.SecurityContextHolder.getContext().setAuthentication(
                new org.springframework.security.authentication.UsernamePasswordAuthenticationToken("operator", "n/a"));
        when(repository.findByResourceKeyAndResourceIdAndActionIdAndActorSubjectAndIdempotencyKey(
                "r", "__collection__", "a", "operator", "key")).thenReturn(Optional.of(winner));
        try {
    ...
    private static ResourceActionExecution execution(String key, String requestHash) {
        return new ResourceActionExecution(java.util.UUID.randomUUID(), "r", "__collection__", "a", "COLLECTION", key, requestHash,
